...
/**
 * Created by steblo on 7/11/2014.
 */
public class BadWorldRaytraceIterator implements Iterable<MovingObjectPosition> {

    private final World world;
    private final Vec3 pos;
    private final Vec3 dir;

    public BadWorldRaytraceIterator(World world, Vec3 pos, Vec3 dir) {
    ...
    public List<MovingObjectPosition> func_147447_a(World world, Vec3 p_147447_1_, Vec3 p_147447_2_, boolean p_147447_3_, boolean p_147447_4_, boolean p_147447_5_)
    {
        Vec3 originalPos = Vec3.createVectorHelper(p_147447_1_.xCoord, p_147447_1_.yCoord, p_147447_1_.zCoord);
        List<MovingObjectPosition> mops = new LinkedList<MovingObjectPosition>();
        ...
        if (!Double.isNaN(p_147447_1_.xCoord) && !Double.isNaN(p_147447_1_.yCoord) && !Double.isNaN(p_147447_1_.zCoord))
        {
            if (!Double.isNaN(p_147447_2_.xCoord) && !Double.isNaN(p_147447_2_.yCoord) && !Double.isNaN(p_147447_2_.zCoord))
            {
                ...
                while (k1-- >= 0)
                {
                    ...
                    {
                        if (p_147447_5_) {
                            mops.add(movingobjectposition2);
                        }
                        //return p_147447_5_ ? movingobjectposition2 : null;
                    }

                    if (p_147447_1_.squareDistanceTo(originalPos) > 25) {
                        return mops;
                    }
                    ...
                    if (!p_147447_4_ || block1.getCollisionBoundingBoxFromPool(world, l, i1, j1) != null)
                    {
                        if (block1.canCollideCheck(l1, p_147447_3_))
                        {
                            ...
                            {
                                mops.add(movingobjectposition1);
    ...
    @Override
    public Iterator<MovingObjectPosition> iterator() {
        List<MovingObjectPosition> movingObjectPositions = func_147447_a(world, pos, dir, false, false, true);
        if (movingObjectPositions.size() > 1) {
            movingObjectPositions.size();
        }
        return movingObjectPositions.iterator();
    }
}
