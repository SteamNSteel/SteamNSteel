...
public class LongPipeRuinWallFeature extends ProceduralWallFeatureBase
{
    ...
    public LongPipeRuinWallFeature(RemnantRuinWallTexture texture, String name, Layer layer)
    {
        super(name, layer);
    ...
    public Collection<FeatureInstance> getFeaturesIn(ChunkCoord chunkCoord)
    {
        ...
        for (int i = 0; i < featureCount; ++i)
        {
            ...
            {
                features.add(new FeatureInstance(this, new BlockPos(xPos, yPos, zPos), width, 1, 1));
            } else
            {
                features.add(new FeatureInstance(this, new BlockPos(zPos, yPos, xPos), 1, 1, width));
            }
        }
        return features;
    }
