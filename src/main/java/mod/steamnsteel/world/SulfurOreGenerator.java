 ...
 * <p/>
 * Here is how it works: <ol> <li> To find a starting place for the vein, take 128 samples. 2/3 of these will be 0 < y <
 * 16 (to find underground lava) and 1/3 of these are 8 < y < 256 (the range used by Mojang for the rare lava lake)
 * </li> <li> If a sample is under lava, laterally neighbors lava or is a ceiling 0 to 3 blocks above lava, it qualifies
 * as a vein starting point. </li> <li> A vein is generated by deciding on a length (of 8 to 24 blocks) and meandering
 * in all 6 directions, stopping if an attempt is made to cross into an unloaded chunk. A vein ignores everything but
 * stone and dirt, and continues to the next block whether sulfur is actually placed or not. </li> </ol> The idea is to
 * bombard a chunk with attempts to start a vein. Restrictions on the qualifications for potential starting points make
 * the vast majority of these attempts fail, but enough attempts are made to statistically ensure the desired quantity
 * of sulfur ore veins. Even though many attempts are made, the performance cost is small because unqualified vein
 * starting points are quickly eliminated.
 * <p/>
 * There are three places that can be adjusted to affect ore frequency: <ol> <li> {@code NUM_ITERATIONS} determines how
 * many vein starts arr attempted pre chunk. </li> <li> {@code NUM_BLOCKS_IN_VEIN} is the desired average length of a
 * vein. </li> <li> {@code yGen} in {@code generate()}: The calculation here determines how many of [NUM_ITERATIONS]
 * attempt to find a common underground lava pool and how many attempt to find a rare (near) surface lava lake. </li>
 * </ol>
 */
